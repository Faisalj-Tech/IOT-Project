...
4. **Memory model diverges at scale** (ADR-0048, arm a): A 500,000-message run published within pinned broker limits (1 GiB memory, 256 MiB alarm threshold) never triggered an alarm, 2.7× past the ~184,000-message prediction. Memory shows a repeating sawtooth pattern (consistent with periodic Raft checkpoint/segment rollover reclaiming the ETS index between measurement pauses) rather than the linear growth the single-burst model fitted on. This phase's own measurement does not validate the design's safety assumption that the broker will alarm before exhausting memory, a consequence most critical for `dlq` — the one queue with no other depth bound at all.
