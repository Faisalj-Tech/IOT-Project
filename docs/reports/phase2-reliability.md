...
**Re-verified post-fix:** two further runs with the fix applied, `C-broker-restart-expC88076.json` and `C-broker-restart-expC88336.json`, both reached 850/850 rows with `gaps: {}` and `verdict: "no-loss"`, confirming the fix holds.
